...
Our configuration library doesn't perfectly map to the Java type system, which means that you can sometimes do silly things by accident. We try to list common error messages here, in alphabetical order of the message itself.
...
The library detects circular type hierarchies caused by return values. For example, if you return the top-level configuration interface from a method inside a subsection.
...
A frequent mistake is to forget `@SubSection` on configuration subsections.
