 ...
 * {@link space.arim.dazzleconf.migration.MigrateSource} is trusted with both loading and detecting of usable old
 ...
 * listener (via {@code MigrateContext#notifyUpdate}). Migration sources areencouraged to flag these updates, even if
 * keys might be arranged by later transitions. {@code UpdateReason.MIGRATED} is an appropriate update reason.
 * <p>
 * <b>Transitions</b>
 * <p>
 * After the old config is loaded, a {@link space.arim.dazzleconf.migration.Transition} transforms it into the latest
 * version. Transitions are designed to be infallible, so that chaining is supported.
 * <p>
 * Like the migration source, the transition can also inform the {@code MigrateContext} if it decided to update any
 * paths. This happens frequently, for example, if keys are moved, if types are changed, or sections are expanded or
 * consolidated. The transition should pass the <i>old path</i> to {@code notifyUpdate} if a key moved,
 * and it should pass multiple old paths (calling the listener each time) if multiple are relevant.
 * <p>
 * <b>Integration and usage</b>
 * <p>
 * A source and a transition together comprise a {@link space.arim.dazzleconf.migration.Migration}. Migrations are
 * addable to a configuration at construction time, and they are used by the <code>configureWith</code> and
 * <code>configureOrFallback</code> methods on {@link space.arim.dazzleconf.Configuration}.
 * <p>
